...
public class OSC : MonoBehaviour
{
        ...
        /*
                this.SetAllMessageHandler((OscMessage message) =>
                {
                    if (Debugging)
                    {
                        ...
                        if (ColorCh != null)
                        {
                            ColorCh.UpdateColors(message);
                        }

                        foreach (ColorChanger ch in ParticleCh)
                        {
                            if (ch != null)
                            {
                                ch.UpdateColors(message);
                            }
                        }
                    }
                });
    ...
    void Start()
    {
      //  GameObject[] p_objects = GameObject.FindGameObjectsWithTag("ParticleColor");

     ///   int p_counter = 0;

       /* foreach (GameObject p in p_objects)
        {
            ParticleCh[p_counter] = p.GetComponent<ColorChanger>();
            p_counter++;
        }
